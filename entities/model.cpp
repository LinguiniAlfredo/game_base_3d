...
Model::Model(const char *model_path, const string &texture_path)
{
...
void Model::render_shadow_map(Shader *shadow_map_shader, const vec3 position, const quat orientation, const vec3 scale)
{
...
void Model::render(Shader *shader, const vec3 position, const quat orientation, const vec3 scale)
{
    for (Mesh &mesh : this->meshes) {
...
vector<Texture> Model::load_material_textures(aiMaterial *mat, aiTextureType type, string type_name)
{
    ...
    //     if (!skip) {
    //         Texture texture;
    //         texture.id = texture_from_file(str);
...
unsigned int Model::texture_from_file(const string &texture_path)
{
